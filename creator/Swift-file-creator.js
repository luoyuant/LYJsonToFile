...
* 获取文件信息
*/
LYFileInfoMetaHandler.prototype.getFileInfoArrayForSwift = function() {
...
* 更新className
*/
LYFileInfoMetaHandler.prototype.updateClassNameForSwift = function() {
	for (var i in this.filesArray) {
		var fileMeta = this.filesArray[i];
		fileMeta.className = this.modelInfoMeta.className;
		this.fileMetaUpdateClassNameForSwift(fileMeta);
	}
	//更新属性类型
	this.updateParentPropertyTypeForSwift();
}

/**
* fileMeta更新关于className的信息
*/
LYFileInfoMetaHandler.prototype.fileMetaUpdateClassNameForSwift = function(fileMeta) {
...
*/
LYFileInfoMetaHandler.prototype.updateParentPropertyTypeForSwift = function(fileMeta) {
	if (!this.modelInfoMeta.isRoot) {
		...
		for (var i in modelInfoArray) {
			...
			for (var j in propertyArr) {
				...
				p.getDefineContentTextForSwift();
			}
			//更新属性
			modelMeta.fileHandler.fileUpdatePropertyTypeForSwift();
		}	
	}
}
...
* 更新属性类型
*/
LYFileInfoMetaHandler.prototype.fileUpdatePropertyTypeForSwift = function() {
	for (var i in this.filesArray) {
		var fileMeta = this.filesArray[i];
		var propertyTextArr = [];
		for (var i in this.modelInfoMeta.propertyInfoArray) {
...
* 文件创建注释信息
*/
LYFileInfoMetaHandler.prototype.getCreatedInfoTextForSwift = function(extensionName) {
	var className = this.modelInfoMeta.className;
	var date = new Date();
...
LYFileInfoMeta.prototype.getDetailInfoForSwift = function() {
...
LYModelPropertyInfoMeta.prototype.getDefineContentTextForSwift = function() {
	//容器泛型
	...
	//Optionals类型
	var optionalsText = usingConfigJSON.setOptionalsForAll ? "?" : "";

	var propertyText = this.name + ": " + this.propertyType + genericText + optionalsText;

	var space = "    ";
	this.defineContentText = space + this.defineKeyword + " " + propertyText;
...
LYCreateFileContentForAllClassForSwift = function(array) {
	var classArr = [];
	var rootFileMeta = {};
